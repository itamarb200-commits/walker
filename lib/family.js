...
// today's calendar date as YYYY-MM-DD in the browser's local time. Good
// enough until Phase 4 threads the family's stored timezone through.
...
export async function toggleLog({ task, slotTime, personId, familyId, isHelp = false }) {
  const supabase = supabaseBrowser();
