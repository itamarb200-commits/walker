...
export async function fetchSettings(familyId) {
  const supabase = supabaseBrowser();
  const [{ data: family }, { data: persons }, { data: pets }, { data: tasks }] = await Promise.all([
    supabase.from("families").select("*").eq("id", familyId).maybeSingle(),
    supabase.from("persons").select("*").eq("family_id", familyId).order("created_at"),
    supabase.from("pets").select("*").eq("family_id", familyId).order("created_at"),
    supabase.from("tasks").select("*").eq("family_id", familyId).order("sort_order"),
  ]);
  return { family, persons: persons || [], pets: pets || [], tasks: tasks || [] };
}
...
export async function addPet(familyId, { name, species }) {
  const supabase = supabaseBrowser();
  const { error } = await supabase.from("pets").insert({ family_id: familyId, name, species });
  if (error) throw error;
}
...
export async function addTask(familyId, task) {
  const supabase = supabaseBrowser();
  const { error } = await supabase.from("tasks").insert({ family_id: familyId, ...task });
  if (error) throw error;
}
